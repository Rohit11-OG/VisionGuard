...
def write_vscode_autostart_files(root: pathlib.Path, script_name: str | None = None) -> None:
    ensure_dir(root / ".vscode")
    # Run VisionGuard as a module so the tasks work regardless of how the
    # package is installed (console script, pip install, or source checkout).
    cmd, base_args = sys.executable, ["-m", "visionguard"]
    tasks = {
        "version": "2.0.0",
        "tasks": [
            {
